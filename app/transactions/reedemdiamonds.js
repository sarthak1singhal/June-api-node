...
module.exports = function(app) {
    ...
    app.post('/redeem', fx.isLoggedIn, async function(req, res) {
        try {
            ...
            console.log( { account_number: accountnumber, fund_account_id: user.razor_fund_account_id, ammount: parseInt(ammount) * 100, currency: 'INR', "mode": "IMPS", purpose: "Points redeemed", "queue_if_low_balance": true } , "=================================scdscs")

            let newPayout = await RazorPayPayout.create({ account_number: accountnumber, fund_account_id: user.razor_fund_account_id, ammount: parseInt(ammount) * 100, currency: 'INR', "mode": "IMPS", purpose: "Points redeemed", "queue_if_low_balance": true }).catch((e) => res.send({ statusCode: 500, message: e, data: {} }));

        ...
        if (fb_id && my_fb_id) {
            ...
            try {
                ...
                if (query1.length != 0) {
                    ...
                    [query99, f] = await acon.execute("select * from videos where fb_id= ? order by id DESC", [fb_id]);
                    console.log(query1)
                    array_out_video = []
                    for (i in query99) {
                    ...
                    //count total heart
                    query123 = await acon.execute("select * from videos where fb_id=? ", [fb_id]);
                    ...
                    [total_following, a] = await acon.execute("SELECT count(*) as count from follow_users where fb_id= ?", [fb_id]);
